...
function Generic({name}){const location=useLocation(); if(name==='reports')return <Reports/>; if(name==='profile')return <SharedProfile/>; if(name==='settings')return location.pathname.startsWith('/super-admin')?<SuperSettings/>:<CompanySettings/>; if(name==='analytics')return <SuperDashboard/>; if(name==='users')return <LiveResourcePage name="employees"/>; if(name==='subscriptions')return <PlatformCompanies/>; return <LiveResourcePage name={name}/>}
...
const company=['company-admins','employees','departments','buildings','floors','rooms','asset-categories','assets','locations','vendors','purchase-orders','invoices','maintenance','asset-requests','asset-allocation','asset-transfers','asset-returns','asset-disposals','repair-history','notifications','reports','audit-logs','settings','profile','ai-assistant'];
const superPages=['users','subscriptions','analytics','reports','notifications','audit-logs','settings','profile']; const employee=['my-assets','request-asset','request-history','notifications','profile','settings','ai-assistant'];
export default function AppRoutes(){return <Routes>
 ...
 <Route path="/super-admin/companies" element={wrap('super',<PlatformCompanies/>)}/><Route path="/super-admin/permissions" element={wrap('super',<Permissions/>)}/><Route path="*" element={<MainLayout><NotFound/></MainLayout>}/></Routes>}
