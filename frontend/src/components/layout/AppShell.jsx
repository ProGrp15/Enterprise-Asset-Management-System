...
import { FaBell, FaBuilding, FaChartBar, FaClipboardList, FaCog, FaFileAlt, FaMapMarkerAlt, FaTools, FaTruck, FaUsers, FaBoxes, FaUserShield, FaSignOutAlt, FaRobot, FaBars, FaSearch, FaChevronDown } from 'react-icons/fa';
import { logout } from '../../store/authSlice';

const icons={Dashboard:FaChartBar,Employees:FaUsers,'Company Admins':FaUserShield,Departments:FaUsers,'Asset Categories':FaBoxes,Assets:FaBoxes,Locations:FaMapMarkerAlt,Vendors:FaTruck,'Purchase Orders':FaFileAlt,Maintenance:FaTools,'Asset Requests':FaClipboardList,'Asset Allocation':FaBoxes,Notifications:FaBell,Reports:FaChartBar,'Audit Logs':FaClipboardList,Settings:FaCog,Profile:FaUsers,Companies:FaBuilding,Users:FaUsers,Subscriptions:FaFileAlt,Analytics:FaChartBar,'My Assets':FaBoxes,'Request Asset':FaClipboardList,'Request History':FaFileAlt,'AI Assistant':FaRobot};
...
export default function AppShell({role,children}) {
  ...
  return <div className="app-layout">
    {open&&<button className="sidebar-scrim d-lg-none" aria-label="Close navigation" onClick={()=>setOpen(false)}/>}<aside className={`app-sidebar ${open?'is-open':''}`}>
      ...
      <div className="sidebar-label">{role==='super'?'Platform control':role==='employee'?'My workspace':'Operations'}</div><nav>{links.map(l=>{const I=icons[l];return <NavLink end={l==='Dashboard'} key={l} to={path(l)} onClick={()=>setOpen(false)} className="app-nav-link"><I/><span>{l}</span>{l==='Notifications'&&<span className="nav-count">3</span>}</NavLink>})}</nav>
    ...
    <main className="app-main"><header className="app-topbar"><div className="topbar-left"><button className="mobile-menu d-lg-none" onClick={()=>setOpen(true)} aria-label="Open navigation"><FaBars/></button><div className="topbar-search"><FaSearch/><input aria-label="Search workspace" placeholder="Search workspace..."/></div></div><div className="topbar-actions"><span className="service-status"><i/> All systems operational</span><button className="icon-button" aria-label="Notifications"><FaBell/><b>3</b></button><div className="user-menu"><div className="avatar">{initials}</div><div className="user-meta"><strong>{name}</strong><span>{user?.role?.replaceAll('_',' ')||'Member'}</span></div><FaChevronDown className="user-chevron"/></div><button aria-label="Log out" className="logout-button" onClick={signOut}><FaSignOutAlt/></button></div></header><div className="app-content">{children}</div></main>
  </div>
}
