...
public class CompanyDataService {
	...
	public List<Map<String, Object>> list(String type, Long company, String q) {
		...
		if (q != null && !q.isBlank()) {
			String col = type.equals("employee") ? "email"
					: type.equals("department") ? "department_name"
							: type.equals("location") ? "location_name" : "company_name";
			sql += " and " + col + " like ?";
			a.add("%" + q + "%");
		}
	...
	public Map<String, Object> one(String type, Long company, Long id) {
	...
	@Transactional
	public Map<String, Object> create(String type, Long company, Map<String, Object> b) {
		switch (type) {
	...
	public void delete(String type, Long company, Long id) {
		String table = table(type), key = key(type);
		owned(table, key, id, company);
		db.update("update " + table + " set is_active=false where " + key + "=?", id);
	}
	@Transactional public Map<String,Object> update(String type, Long company, Long id, Map<String,Object> b) {
