...
@RestControllerAdvice public class GlobalExceptionHandler {
  ...
  @ExceptionHandler(DataIntegrityViolationException.class) ResponseEntity<Map<String,Object>> conflict(DataIntegrityViolationException e){log.warn("Data integrity conflict", e);return ResponseEntity.status(409).body(Map.of("success",false,"message","A record with the same unique value already exists or a relationship is invalid."));}
  @ExceptionHandler(Exception.class) ResponseEntity<Map<String,Object>> generic(Exception e){log.error("Unhandled asset-service request failure", e);return ResponseEntity.internalServerError().body(Map.of("success",false,"message","The request could not be completed."));}
}
